...
def process_session_pipeline(uploaded_file_path: str, session_folder: str):
    ...
    Args:
        uploaded_file_path (str): The path to the user-uploaded file.
        session_folder (str): The unique folder for this session.

    Yields:
        str: Status messages indicating the progress of the pipeline.
    """
    # Step 1: Prepare Audio (handles video extraction)
    yield "Step 1/4: Preparing audio from uploaded file..."
    ...
    yield "Step 3/4: Transcribing audio..."
    transcript_path = run_transcription(audio_path, timeline_path, session_folder)
    if not transcript_path:
        yield "ERROR: Transcription failed."
        return

    # Step 4: Indexing
    yield "Step 4/4: Indexing transcript for Q&A..."
...
def ask_question(question: str, session_folder: str):
    ...
    if not results['documents'] or results['distances'][0][0] > 1.3:
        return "I could not find a relevant answer in the provided transcript to answer that question."
    ...
    prompt_template = """
         ...
         3.  After your answer, cite the speaker and the time range of the source(s) you used in the format.
    ...
    # 7. Query Ollama and stream the response
